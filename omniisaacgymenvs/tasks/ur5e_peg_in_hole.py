...
class UR5eCabinetTask(RLTask):
    ...
    def init_data(self) -> None:
        ...
        hand_pose = get_env_local_pose(self._env_pos[0], UsdGeom.Xformable(stage.GetPrimAtPath("/World/envs/env_0/UR5e/panda_link7")), self._device)
        lfinger_pose = get_env_local_pose(
            self._env_pos[0], UsdGeom.Xformable(stage.GetPrimAtPath("/World/envs/env_0/UR5e/panda_leftfinger")), self._device
        )
        rfinger_pose = get_env_local_pose(
            self._env_pos[0], UsdGeom.Xformable(stage.GetPrimAtPath("/World/envs/env_0/UR5e/panda_rightfinger")), self._device
        )
        ...
        grasp_pose_axis = 1
        UR5e_local_grasp_pose_rot, UR5e_local_pose_pos = tf_combine(hand_pose_inv_rot, hand_pose_inv_pos, finger_pose[3:7], finger_pose[0:3])
        UR5e_local_pose_pos += torch.tensor([0, 0.04, 0], device=self._device)
        self.UR5e_local_grasp_pos = UR5e_local_pose_pos.repeat((self._num_envs, 1))
        self.UR5e_local_grasp_rot = UR5e_local_grasp_pose_rot.repeat((self._num_envs, 1))
        ...
        self.UR5e_default_dof_pos = torch.tensor(
    ...
    def get_observations(self) -> dict:
        hand_pos, hand_rot = self._UR5es._hands.get_world_poses(clone=False)
        drawer_pos, drawer_rot = self._cabinets._drawers.get_world_poses(clone=False)
        UR5e_dof_pos = self._UR5es.get_joint_positions(clone=False)
        UR5e_dof_vel = self._UR5es.get_joint_velocities(clone=False)
        ...
        self.UR5e_dof_pos = UR5e_dof_pos

        self.UR5e_grasp_rot, self.UR5e_grasp_pos, self.drawer_grasp_rot, self.drawer_grasp_pos = self.compute_grasp_transforms(
            hand_rot,
            hand_pos,
            self.UR5e_local_grasp_rot,
            self.UR5e_local_grasp_pos,
            drawer_rot,
            drawer_pos,
        ...
        self.UR5e_lfinger_pos, self.UR5e_lfinger_rot = self._UR5es._lfingers.get_world_poses(clone=False)
        self.UR5e_rfinger_pos, self.UR5e_rfinger_rot = self._UR5es._lfingers.get_world_poses(clone=False)

        dof_pos_scaled = (
            2.0
            * (UR5e_dof_pos - self.UR5e_dof_lower_limits)
            / (self.UR5e_dof_upper_limits - self.UR5e_dof_lower_limits)
            - 1.0
        )
        to_target = self.drawer_grasp_pos - self.UR5e_grasp_pos
        self.obs_buf = torch.cat(
            (
                dof_pos_scaled,
                UR5e_dof_vel * self.dof_vel_scale,
                to_target,
    ...
    def pre_physics_step(self, actions) -> None:
        ...
        targets = self.UR5e_dof_targets + self.UR5e_dof_speed_scales * self.dt * self.actions * self.action_scale
        self.UR5e_dof_targets[:] = tensor_clamp(targets, self.UR5e_dof_lower_limits, self.UR5e_dof_upper_limits)
    ...
    def reset_idx(self, env_ids):
        ...
        pos = tensor_clamp(
            ...
            + 0.25 * (torch.rand((len(env_ids), self.num_UR5e_dofs), device=self._device) - 0.5),
    ...
    def post_reset(self):
        ...
        self.UR5e_dof_pos = torch.zeros((self.num_envs, self.num_UR5e_dofs), device=self._device)
        dof_limits = self._UR5es.get_dof_limits()
        self.UR5e_dof_lower_limits = dof_limits[0, :, 0].to(device=self._device)
        self.UR5e_dof_upper_limits = dof_limits[0, :, 1].to(device=self._device)
        self.UR5e_dof_speed_scales = torch.ones_like(self.UR5e_dof_lower_limits)
        self.UR5e_dof_speed_scales[self._UR5es.gripper_indices] = 0.1
    ...
    def calculate_metrics(self) -> None:
        self.rew_buf[:] = self.compute_UR5e_reward(
            self.reset_buf, self.progress_buf, self.actions, self.cabinet_dof_pos,
            self.UR5e_grasp_pos, self.drawer_grasp_pos, self.UR5e_grasp_rot, self.drawer_grasp_rot,
            self.UR5e_lfinger_pos, self.UR5e_rfinger_pos,
            ...
            self.finger_dist_reward_scale, self.action_penalty_scale, self.distX_offset, self._max_episode_length, self.UR5e_dof_pos,
    ...
    def compute_grasp_transforms(
        ...
        return global_UR5e_rot, global_UR5e_pos, global_drawer_rot, global_drawer_pos

    def compute_UR5e_reward(
        self, reset_buf, progress_buf, actions, cabinet_dof_pos,
        UR5e_grasp_pos, drawer_grasp_pos, UR5e_grasp_rot, drawer_grasp_rot,
        UR5e_lfinger_pos, UR5e_rfinger_pos,
        ...
        d = torch.norm(UR5e_grasp_pos - drawer_grasp_pos, p=2, dim=-1)
        dist_reward = 1.0 / (1.0 + d ** 2)
        dist_reward *= dist_reward
        dist_reward = torch.where(d <= 0.02, dist_reward * 2, dist_reward)

        axis1 = tf_vector(UR5e_grasp_rot, gripper_forward_axis)
        axis2 = tf_vector(drawer_grasp_rot, drawer_inward_axis)
        axis3 = tf_vector(UR5e_grasp_rot, gripper_up_axis)
        ...
        around_handle_reward = torch.where(UR5e_lfinger_pos[:, 2] > drawer_grasp_pos[:, 2],
                                           torch.where(UR5e_rfinger_pos[:, 2] < drawer_grasp_pos[:, 2],
                                                       around_handle_reward + 0.5, around_handle_reward), around_handle_reward)
        # reward for distance of each finger from the drawer
        finger_dist_reward = torch.zeros_like(rot_reward)
        lfinger_dist = torch.abs(UR5e_lfinger_pos[:, 2] - drawer_grasp_pos[:, 2])
        rfinger_dist = torch.abs(UR5e_rfinger_pos[:, 2] - drawer_grasp_pos[:, 2])
        finger_dist_reward = torch.where(UR5e_lfinger_pos[:, 2] > drawer_grasp_pos[:, 2],
                                         torch.where(UR5e_rfinger_pos[:, 2] < drawer_grasp_pos[:, 2],
        ...
        # rewards = torch.where(UR5e_lfinger_pos[:, 0] < drawer_grasp_pos[:, 0] - distX_offset,
        #                       torch.ones_like(rewards) * -1, rewards)
        # rewards = torch.where(UR5e_rfinger_pos[:, 0] < drawer_grasp_pos[:, 0] - distX_offset,
        #                       torch.ones_like(rewards) * -1, rewards)
